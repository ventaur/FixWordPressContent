...
function buildUrlRegex(buildOptions, customUrl = null) {
    ...
    if (customUrl) {
        if (!(customUrl instanceof RegExp) && typeof customUrl !== 'string') {
            throw new Error('If provided, customUrl must be either a RegExp or string.');
        }
    } else {
    ...
    if (customUrl) {
        regexPatterns.push(typeof customUrl === 'string'
            ? customUrl
            : customUrl.source
        );
    } else {
